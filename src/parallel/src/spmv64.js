  ...
  for(var i = 0; i < A_dia.ndiags; i++){
    for(var j = 0; j < A_dia.nrows; j++){
      if (data[i * A_dia.nrows + j] != 0)
        console.log(j, offset[i] + j, data[i*A_dia.nrows + j], A_dia.data_index + 8 * (i * A_dia.nrows + j));
    }
  }
}
  ...
  for(var i = 0; i < A_dia.nrows; i++){
    for(var j = 0; j < A_dia.ndiags; j++){
      if (data[i * A_dia.ndiags + j] != 0)
        console.log(i, i + offset[j], data[i*A_dia.ndiags + j], A_dia.data_index + 8 * (i * A_dia.ndiags + j));
    }
  }
}
