  ...
  (func $spmv_csr (export "spmv_csr") (param $csr_rowptr i32) (param $csr_col i32) (param $csr_val i32) (param $x i32) (param $y i32) (param $len i32)
	...
	(else
	  ...
	  (then
          ...
          (f32.load (local.get $y))
          (f32.add)
          (f32.load (i32.add (local.get $csr_val) (i32.const 4)))
          (i32.add (local.get $x) (i32.shl (i32.load (i32.add (local.get $csr_col) (i32.const 4))) (i32.const 2)))
          (f32.load)
          (f32.mul)
          (f32.add)
	  (f32.store)
          (local.set $csr_col (i32.add (local.get $csr_col) (i32.const 8)))
	...
	(else
	  ...
	  (if
	    ...
	    (local.set $csr_col (i32.add (local.get $csr_col) (i32.const 8)))
	    (local.set $csr_val (i32.add (local.get $csr_val) (i32.const 8)))
            (local.tee $j (i32.add (local.get $j) (i32.const 2)))
