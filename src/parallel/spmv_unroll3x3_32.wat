  ...
  (func $spmv_csr (export "spmv_csr") (param $csr_rowptr i32) (param $csr_col i32) (param $csr_val i32) (param $x i32) (param $y i32) (param $len i32)
	...
	(then
 	  ...
 	  (then
 	    ...
 	    (then
            ...
            (loop $inner_loop_001
              ...
              (local.set $csr_col (i32.add (local.get $csr_col) (i32.const 4)))
              (local.set $csr_val (i32.add (local.get $csr_val) (i32.const 4)))
              (local.tee $third (i32.add (local.get $third) (i32.const 1)))
              (local.get $j)
              (i32.ne)
              (br_if $inner_loop_001)
            )
            (local.get $y3)
            (local.get $temp3)
            (f32.store)
	  ...
	  (local.set $j3)
          ...
          (f32.load (local.get $y3))
          (local.set $temp3)
          (local.set $csr_col3 (i32.add (local.get $csr_col) (i32.shl (local.get $j2) (i32.const 2))))
          (local.set $csr_val3 (i32.add (local.get $csr_val) (i32.shl (local.get $j2) (i32.const 2))))
          (i32.const 0)
          (local.set $j)
          (loop $inner_loop_jam_011
            (f32.load (local.get $csr_val))
            (i32.add (local.get $x) (i32.shl (i32.load (local.get $csr_col)) (i32.const 2)))
            (f32.load)
            (f32.mul)
	...
	(local.get $j1)
        ...
        (if
          ...
          (loop $inner_loop_peel_111_3
            ...
            (local.set $csr_col3 (i32.add (local.get $csr_col3) (i32.const 4)))
            (local.set $csr_val3 (i32.add (local.get $csr_val3) (i32.const 4)))
            (local.tee $j1 (i32.add (local.get $j1) (i32.const 1)))
            (local.get $j3)
            (i32.ne)
            (br_if $inner_loop_peel_111_3)
          )
          )
        )
        (local.get $y)
